...
def get_args_parser():
    ...
    parser.add_argument('--hoi_concept', default=1, type=int) # hoi_concept_discovery
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_max_norm, hoi_concept=args.hoi_concept)
        lr_scheduler.step()
        if args.output_dir:
        ...
        if args.hoi:
            test_stats = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val, args.subject_category_id, device)
            if output_dir is not None and args.hoi_concept:
                import os
                os.system("PYTHONPATH=. python scripts/analysis_attentions.py {}/checkpoint.pth".format(output_dir))
            coco_evaluator = None
        else:
