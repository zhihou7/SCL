...
class DETRHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False):
        ...
        self.affordance_matrix = nn.Parameter(torch.zeros([num_verb_classes,
                                                           num_obj_classes + 1]),
                                              requires_grad=False)
        self.affordance_count = nn.Parameter(torch.zeros([1]), requires_grad=False)
        self.affordance_count1 = nn.Parameter(torch.zeros([num_verb_classes,
                                                       num_obj_classes + 1]), requires_grad=False)
    ...
    def stat_concept_confidence(self, out_verb_prob, out_obj_prob, target=None):
        out_obj_prob = out_obj_prob.unsqueeze(1)
        out_verb_prob = out_verb_prob.unsqueeze(-1)
        matrix = torch.matmul(out_verb_prob, out_obj_prob)
        matrix = torch.square(matrix)  # normalize
        length = len(matrix)
        if target is not None:
            target_classes_onehot = torch.zeros([target.shape[0], target.shape[1], self.num_obj_classes + 1],
                                                dtype=target.dtype, layout=target.layout, device=target.device)

            target_classes_onehot.scatter_(2, target.unsqueeze(-1), 1)
            target_classes_onehot = target_classes_onehot.view(len(out_obj_prob), -1)
            target_classes_onehot = torch.repeat_interleave(target_classes_onehot.unsqueeze(1), matrix.shape[1], dim=1)
            matrix = target_classes_onehot * matrix

            total_count = self.affordance_count1.data + torch.sum(target_classes_onehot.type(torch.float32), dim=0, keepdim=False)
            self.affordance_matrix.data = (torch.mean(matrix, dim=0, keepdim=False) +
                                           self.affordance_matrix.data * self.affordance_count1.data) / (total_count + (total_count == 0).type(target.dtype))
            self.affordance_count1.data = total_count
            # print(self.affordance_count1.max(), self.affordance_matrix.max())
        else:
            self.affordance_matrix.data = (torch.mean(matrix, dim=0, keepdim=False) +
                                           self.affordance_matrix.data * self.affordance_count.data) / (self.affordance_count.data + length)
            self.affordance_count.data = self.affordance_count.data + length
